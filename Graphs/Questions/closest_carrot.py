...
def closest_carrot(matrix, row, col): # Breadth First Search
    ...
    COL = len(matrix[0])
    visit = set()
    queue = deque([(row, col, 0)]) # Store the row, column, and distance from the starting position
    directions = [(0, 1), (1, 0), (-1, 0), (0, -1)] # Store the directions to move

    while queue:
        r, c, distance = queue.popleft() # Get the row, column, and distance from the queue
        ...
        for x, y in directions: # Iterate over the directions
            dx, dy = r + x, c + y
            queue.append((dx, dy, distance + 1)) # Add the row, column, and distance to the queue
